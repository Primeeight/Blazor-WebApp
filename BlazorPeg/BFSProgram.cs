...
public partial class BFSProgram
{
        ...
        //Generate a new trinagle from the given s ize.
        ...
        public (string, string)[] getMovesString()
        {
                //create new dictionary.
                //dictionary stringMoves
                Dictionary<(int, int), string> moveStrings = new Dictionary<(int, int), string>();

                //int aIndex = 65;
                int aIndex = 65;
                //(string, string)[] stringPath = new (string, string)[movePath.length()];
                (string, string)[] stringPath = new (string, string)[movePath.Count];

                //foreach (move in Moves)
                foreach (var key in adjList.Keys)
                {
                        //stringmoves(move.getRepTrunc) = (alphIndex).toString();
                        moveStrings.Add(key, ("" + Convert.ToChar(aIndex)));
                        //aIndex ++;
                        aIndex++;
                ...
                {
                        //var rep = move.getRepTrunc;
                        var rep = movePath[i].GetRepTrunc();
                        (string, string) newRep = (moveStrings[rep.Item1], moveStrings[rep.Item2]);
                        //(string, string) newRep = (stringMoves[newRep.item1],stringMoves[newRep.item2]);
                        //stringPath[i] = newRep;
                        stringPath[i] = newRep;
                }
